...
Idea: per state bar graph with percentage of diversity on student that goe to college


##Step 1: Wrangle data
```{r}
total_state_enrrolment <- diversity_school %>% 
  group_by(state, category) %>% 
  summarise(total_category = sum(enrollment)) %>% 
  ungroup() %>% 
  group_by(state) %>% 
  mutate(state_enrolment = sum(total_category))
...
##Plot
```{r}
college_diversity_plot <- ggplot(total_state_enrrolment %>% filter(!is.na(state)))+
  ...
  scale_color_paletteer_d("ochRe::mccrea")
...
```{r}
california <- filter(diversity_school, state== "California")


cali_women <- california %>% 
  filter(category == "Women")

##How does tutition vary on the west coast states?
